...
function NoteInterviewAnswer({ answer, className }: NoteInterviewAnswerProps) {
  return (
    <section
      className={cn(
        'rounded-[1.5rem] border border-border/70 bg-accent-soft p-5 shadow-[0_22px_64px_-48px_color-mix(in_oklch,var(--primary),transparent_25%)] md:p-6',
        className,
      )}
    >
      ...
      <p className="mt-4 text-[0.98rem] leading-7 text-foreground md:text-[1.02rem]">{answer}</p>
    </section>
  )
}
